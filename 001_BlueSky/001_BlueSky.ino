/*
   ...
   A meditation on the sky and Perlin Noise :)

   Code for noise from: https://forum.arduino.cc/index.php?topic=444299.0

   Additional resource on Perlin noise - http://mrl.nyu.edu/~perlin/noise/
...
Color SKY_BLUE = makeColorHSB(127, 255, 255);

byte cloud_one_pos = 0;
byte cloud_two_pos = 0;
...
void loop() {
  ...
  x = float(millis()) / 1000.0f;
  
  FOREACH_FACE(f) {
    p[f] = fastFloor(128 * generate(x + f) + 128);
    setColorOnFace(makeColorHSB(127, p[f], 255), f);
  }
